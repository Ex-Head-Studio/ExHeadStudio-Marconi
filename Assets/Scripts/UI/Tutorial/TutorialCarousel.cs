...
{
    [Header("Tutorial Carousel Settings")]
    ...
    [SerializeField] private float floatAmplitude = 10f; // Ampiezza del movimento oscillatorio del sottomarino
    [SerializeField] private float floatFrequency = 1f; // Frequenza del movimento oscillatorio del sottomarino
    public bool isCarouselActive = true; // Flag to control carousel activation
    private Vector2 subBasePosition; // posizione fissa di riferimento
    ...
    void Start()
    {
        ...
        if (submarineIndicator != null)
            subBasePosition = submarineIndicator.anchoredPosition;
    ...
    void Update()
    {
        if (isCarouselActive && submarineIndicator != null)
        {
            Vector2 pos = submarineIndicator.anchoredPosition;
            pos.y = subBasePosition.y + Mathf.Sin(Time.time * floatFrequency * 2f * Mathf.PI) * floatAmplitude;
            submarineIndicator.anchoredPosition = new Vector2(pos.x, pos.y);
        }
    }
