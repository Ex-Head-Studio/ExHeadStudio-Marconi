    ...
    private void ShowStatsPanel(DisplayStatsClass displayStats)
    {
        if (!isDisplaying)
        {
            ...
            if (displayStats.script is AShip)
            {
                ...
                SetShipClass(shipScript.GetShipSO());
                //_objectDescription.text = shipScript.GetShipSO().shipDescription;
                ...
                if (shipScript.shipSO.shipModelMesh != null)
                {
                    ...
                    Vector3 repositionOffset = shipScript.shipSO.uiRepositionOffset;  // Regola questi valori secondo necessità
                    Vector3 scaleFactor = shipScript.shipSO.uiScaleFactor;     // Regola questi valori secondo necessità
